...
def convert_to_rgb(images):
    converted_rgb = np.empty((len(images),1024))
    with tqdm(total=len(images)) as pbar:
        for imgnum in range(0, len(images)):
            pbar.update(1)
            temp = []
            grey = np.zeros((images[imgnum].shape[0], images[imgnum].shape[1])) # init 2D numpy array

            for rownum in range(len(images[imgnum])):
               for colnum in range(len(images[imgnum][rownum])):
                  grey[rownum][colnum] = weightedAverage(images[imgnum][rownum][colnum])
            grey = grey.reshape((1, 1024))
            converted_rgb[imgnum] = grey
...
def precision(y_true, y_pred): #From keras legacy source code
...
def recall(y_true, y_pred): #From keras legacy source code
...
def fbeta_score(y_true, y_pred, beta=1): #From keras legacy source code
...
if __name__ == "__main__":
    ...
    generator.add(Dense(kernel_initializer="glorot_uniform", input_dim=z_input_vector, units=2000))
    generator.add(BatchNormalization())
    generator.add(LeakyReLU(alpha=0.3))
    generator.add(Dense(1700, kernel_initializer="glorot_uniform"))
    generator.add(BatchNormalization())
    generator.add(LeakyReLU(alpha=0.3))
    generator.add(Dense(1500, kernel_initializer="glorot_uniform"))
    generator.add(BatchNormalization())
    generator.add(LeakyReLU(alpha=0.3))
    generator.add(Dense(1024, kernel_initializer="glorot_uniform", activation="sigmoid"))
    generator.compile(loss='binary_crossentropy', optimizer=generator_optimizer)
    generator.summary()

    # Discriminator Model
    discriminator = Sequential()
    discriminator.add(Dense(kernel_initializer="glorot_uniform", units=240, input_dim=1024))
    ...
    if not os.path.isfile('saved_cigar10_train.pkl'):
        print('converting X_train to grayscale')
        X_train = convert_to_rgb(X_train)
        print('converting X_test to grayscale')
        X_test = convert_to_rgb(X_test)
        with open('saved_cigar10_train.pkl', 'wb') as fid:
            pickle.dump(X_train, fid)
        with open('saved_cigar10_test.pkl', 'wb') as fid:
            pickle.dump(X_test, fid)
    else:
        with open('saved_cigar10_train.pkl', 'rb') as fid:
            X_train = pickle.load(fid)
        with open('saved_cigar10_test.pkl', 'rb') as fid:
    ...
    for i in range(9):
        plt.subplot(3, 3, i+1)
        img = X_train[random_mnist_idx[i], :].reshape((32, 32))
        plt.imshow(img, cmap='gray')
        plt.axis('off')
    ...
    for i in range(0, 9):
        plt.subplot(3, 3, i+1)
        x = initial_generated_images[i].reshape((32, 32))
